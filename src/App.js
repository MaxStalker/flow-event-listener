...
import FlowListener from './event-listener'
...
const deployHelloCadence = async () => {
  const code = `
    access(all) contract HelloWorld {
      ...
      pub event SuperEvent(number: Int)
  ...
  const tx = await fcl.send([
    sdk.transaction`
        ...
        `,
    fcl.args([fcl.arg(Buffer.from(code, 'utf8').toString('hex'), types.String)]),
...
const getEvents = async (params, type) => {
  ...
  console.log(`Fetch events with type "${eventType}"`)
...
function App() {
  ...
  useEffect(() => {
    const ear = new FlowListener({
      dispatch: (event) => {
        const { eventType, data } = event
        console.log(`Event of type "${eventType}":`, data)
      },
      pollInterval: 1000,
    })
    ear.subscribe()
    return ear.unsubscribe
  }, [])
    ...
    <div>
      ...
      {!userLoggedIn ? (
        ...
        <>
          ...
          <button onClick={emitSuper}>Emit Super</button>
